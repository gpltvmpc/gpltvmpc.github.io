function copyBibTeX() {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    //   3000
    // );
    const imageCarouselSingle1 = new CarouselSingle(
      document.querySelector("#imageCarouselSingle"),
      10000
    );
    const imageCarouselSingle2 = new CarouselSingle(
      document.querySelector("#imageCarouselSingle2"),
      10000
    );
    const imageCarouselSingle3 = new CarouselSingle(
      document.querySelector("#imageCarouselSingle3"),
      10000
    );
    ...
    // Add touch support
    // const carousels = [imageCarousel, videoCarousel, imageCarouselSingle];
    const carousels = [imageCarouselSingle1, imageCarouselSingle2, imageCarouselSingle3];
...
class CarouselSingle {
  constructor(element, interval = 3000) {
    this.container = element;
    this.track = element.querySelector(".carousel-single-track");
    this.slides = Array.from(element.querySelectorAll(".carousel-single-slide"));
    this.indicators = element.querySelector(".carousel-single-indicators");

    this.currentIndex = 0;
    this.slidesPerView = 1;                          // <- one at a time
    this.totalSlides = this.slides.length;           // <- no grouping needed
    this.interval = interval;
    this.autoPlayTimer = null;
  ...
  createIndicators() {
    this.indicators.innerHTML = "";                  // reset if reinit
    for (let i = 0; i < this.totalSlides; i++) {
      const button = document.createElement("button");
      button.classList.add("indicator");
      if (i === 0) button.classList.add("active");
